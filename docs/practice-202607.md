...
| # | 错误 | 说明 |
|:-:|------|------|
| 1 | **完全没实现 LIS** | 这不是 DP 也不是贪心，没有维护子序列信息，只是统计了部分递增对的数量 |
| 2 | `count` 的含义错误 | `count` 统计的是满足 `i<j 且 nums[i]<nums[j]` 的**数对数量**，不是**子序列长度**。示例中本应输出 4，这段代码输出 5 |
| 3 | 内层循环条件诡异 | `j < nums.length - (j - i)` 中 `(j-i)` 随 j 增大而增大，导致循环提前终止，逻辑完全不可控 |
| 4 | 不连续的要求没体现 | 即使统计了递增对，也没有把它们串成一个**序列** |
